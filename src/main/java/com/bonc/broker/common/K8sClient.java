...
public class K8sClient {
	...
	public static KubernetesClient getK8sClient() {
		...
		if (null == kubernetesClient) {
			logger.info("---masterURL value from application-dev.properties:\t" + PropertyUtil.getProperty("master.url"));
			Config config;
			try {
				//"http://172.16.3.30:23333"
				System.out.println("=======================>" + PropertyUtil.getProperty("master.url"));
				config = new ConfigBuilder().withMasterUrl(PropertyUtil.getProperty("master.url")).build();
			} catch (Exception e) {
